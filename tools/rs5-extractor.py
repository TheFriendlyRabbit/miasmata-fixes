        ...
        if list_chunks and file.type not in ('PROF', 'INOD', 'FOGN'):
            try:
                ...
                for chunk in chunks.values():
                    print(
                        '%4s %8s - %4s %8i' % ('', '', chunk.name, chunk.size))
            print()


def extract(archive, dest, file_list, strip, chunks, overwrite, filter):
    rs5 = rs5archive.Rs5ArchiveDecoder(open(archive, 'rb'))
    print(f'Extracting files to {dest}')
    if not file_list:
        file_list = rs5
    for filename in file_list:
        filename = filename.replace(os.path.sep, '\\')
        ...
        if filter and type not in filter:
            continue
        try:
            print(f'Extracting {repr(type)} {filename}...')
            if chunks:
            ...
            print(
                f'ERROR EXTRACTING {filename}: {e}, SKIPPING!',
                file=sys.stderr)
        ...
        if os.path.isdir(filename):
            ...
            for (root, dirs, files) in os.walk(filename):
                ...
                for file in files:
                    ...
                    if rs5mod.file_blacklisted(
                            os.path.relpath(path, filename).replace(os.path.sep,
                                                                    '\\')):
...
def create_rs5(archive, file_list, overwrite):
    ...
    if os.path.exists(archive) and not overwrite:
        print(f'{archive} already exists, refusing to continue!')
        return
...
def repack_rs5(old_archive, new_archive):
    ...
    for old_file in old_rs5.values():
        if rs5mod.file_blacklisted(old_file.filename):
            print(f'Discarding {old_file.filename}')
            continue
        print(f'Repacking {old_file.filename}...')
...
def analyse(filename):
    ...
    for file in rs5.values():
        if not file.filename:
            # XXX: What are these?
            # print 'SKIPPING FILE OF TYPE %s WITHOUT FILENAME' % repr(
            # file.type)
            continue
        try:
            d = file.decompress()
            ...
            # XXX: What are these?
            print(f'ERROR EXTRACTING {file.filename}: {str(e)}')
            print('%s %x %8i %x    |   %-25s  |  compressed_size: %i' \
                  % (file.type, file.u1, file.uncompressed_size, file.u2,
                     file.filename, file.compressed_size))
        ...
        if True:
            print(
                '0x%.8x - 0x%.8x  |  %s %x %8i %x %x  |   %-25s  |  '
                'compressed_size: %i\t|  size: %8i' \
                % (file.data_off, file.data_off + file.compressed_size - 1,
                   file.type, file.u1,
                   file.uncompressed_size, file.u2, contents.u2, file.filename,
                   file.compressed_size,
                   contents.filesize + contents.data_off))
        ...
        # ALIGNMENT CONSTRAINT FOUND - FILE IS PADDED TO 8 BYTES BEFORE
        # COMPRESSION
...
def parse_args():
    ...
    group.add_argument('-L', '--list-chunks', action='store_true',
                       help='List all files and contained chunks in the rs5 '
                            'archive')
    group.add_argument('--sha1', action='store_true',
                       help='Calculate the sha1sum of the zlib compressed '
                            'version of all files within the archive')
    group.add_argument('--list-holes', action='store_true',
                       help='List the location of each file or structure in '
                            'the archive, and any holes between them')
    ...
    group.add_argument('--add-mod', action='store_true',
                       help='Merge mods specified by FILEs into ARCHIVE with '
                            'undo metadata')
    group.add_argument('--rm-mod', '--remove-mod', action='store_true',
                       help='Remove a mod previously added with --add-mod')
    group.add_argument('--order', '--mod-order', '--set-mod-order',
                       action='store_true',
                       help='Change the order of mods added with --add-mod '
                            'within the archive')
    group.add_argument('--repack', metavar='NEW_ARCHIVE',
                       # TODO: Discard UNDO metadata
                       help='Decode ARCHIVE and pack into NEW_ARCHIVE, '
                            'for testing')

    group.add_argument('--add-undo', action='store_true',
                       help='Add undo metadata to an rs5 archive (WARNING: '
                            'Not all actions can be undone)')
    group.add_argument('--revert', action='store_true',
                       help='Use undo metadata in an rs5 archive to restore '
    ...
    group1.add_argument('--chunks', action='store_true',
                        help='Split files up into their component chunks '
                             'while extracting')
    ...
    if args.extract:
        ...
        if directory is None:
            ...
            if directory == args.file:
                ...
                return
        return extract(args.file, directory, args.files, args.strip,
                       args.chunks, args.overwrite, args.filter)
