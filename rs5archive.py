...
def to_win_time(unix_time):
    return int((unix_time - win_epoch) * 10000000)
...
class Rs5CompressedFile(object):
    def gen_dir_ent(self):
        ...
        return struct.pack('<QIQ4sQQ',
                           ...
                           to_win_time(self.modtime)) + self.filename + b'\0'
...
class Rs5CompressedFileDecoder(Rs5CompressedFile):
    def __init__(self, f, data):
        self.fp = f
        (self.data_off, self.compressed_size, self.u1, self.type,
        ...
        filename_len = data[40:].find(b'\0')
        self.filename = data[40:40 + filename_len].decode("utf-8")
        self.modtime = from_win_time(modtime)

    def extract(self, base_path, strip, overwrite):
        dest = os.path.join(base_path, self.filename.replace('\\',
                                                             os.path.sep))
        if os.path.isfile(dest) and not overwrite:  # and size != 0
            print(f'Skipping {dest} - file exists.', file=sys.stderr)
            return
        ...
        try:
            ...
            if strip:
                ...
                # assert(len(contents.data) == filesize) # Removed because it
                # breaks --strip
                f.write(contents.data)
            else:
                f.write(data)
        except zlib.error as e:
            print('ERROR EXTRACTING %s: %s. Skipping decompression!' % (dest,
                                                                        str(e)),
                  file=sys.stderr)
            f.write(self.read())
        f.close()
    ...
    def extract_chunks(self, base_path, overwrite):
        ...
        dest = dest.rstrip()  # "TEX/Rock_Set2_Moss " ends in a space,
        # which Windows can't handle, so strip it.
        data = self.decompress()

        try:
            chunks = rs5file.Rs5ChunkedFileDecoder(data)
        except:
            # print('NOTE: %s does not contain chunks, extracting whole
            # file...' % dest, file=sys.stderr)
            return self.extract(base_path, False, overwrite)

        if os.path.exists(dest) and not os.path.isdir(dest):
            print(
                'WARNING: %s exists, but is not a directory, skipping!' % dest,
                file=sys.stderr)
            return
        ...
        for (i, chunk) in enumerate(iter(chunks.values()), 1):
            extension = (self.type, chunk.name)
            path = os.path.join(dest, '%.2i-%s%s' % (i, chunk.name,
                                                     chunk_extensions.get(
                                                         extension, '')))
            if os.path.isfile(path) and not overwrite:  # and size != 0
                print('Skipping %s - file exists.' % dest, file=sys.stderr)
                continue
...
class Rs5CentralDirectoryDecoder(Rs5CentralDirectory):
    def __init__(self, real_fp=None):
        ...
        for f_off in range(self.d_off + self.ent_len,
                           self.d_off + self.d_orig_len, self.ent_len):
            try:
                entry = Rs5CompressedFileDecoder(real_fp,
                                                 self.fp.read(self.ent_len))
...
class Rs5CentralDirectoryEncoder(Rs5CentralDirectory):
    def write_directory(self):
        ...
        pad = b'\0' * (self.ent_len - len(
            dir_hdr))  # XXX: Not sure if any data here is important
        self.fp.write(dir_hdr + pad)

        for file in self.values():
            ent = file.gen_dir_ent()
            pad = b'\0' * (self.ent_len - len(
                ent))  # XXX: Not sure if any data here is important
...
class Rs5ArchiveEncoder(Rs5CentralDirectoryEncoder):
    ...
    def add(self, filename, seek_cb=None, progress=progress):
        progress(msg=RS5Patcher.tr(f"Adding {filename}..."))
    ...
    def add_from_buf(self, buf, seek_cb=None, progress=progress):
        ...
        progress(msg=RS5Patcher.tr(f"Adding {entry.filename}..."))
        self[entry.filename] = entry

    def add_chunk_dir(self, path, seek_cb=None):
        print(f"Adding chunks from {path}...")
        ...
        for filename in files:
            ...
            if not os.path.isfile(chunk_path) or '-' not in filename:
                print(f'Skipping {chunk_path}: not a valid chunk')
                continue
        ...
        chunks = rs5file.Rs5ChunkedFileEncoder(header.magic, header.filename,
                                               header.u2, chunks)
    ...
    def write_header(self, progress=progress):
        ...
        self.fp.seek(0)
        self.fp.write(
            struct.pack('<8sQII', b'CFILEHDR', self.d_off, self.ent_len,
                        self.u1))
    ...
    def do_timestamp_workaround(self, progress=progress):
        ...
        progress(msg=RS5Patcher.tr(
            "Setting timestamp on %s to %s to workaround for v2.0.0.4 bug" % \
