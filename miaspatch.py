...
import environment
import miasmod
from lib import miasutil, rs5archive, rs5mod
    ...
    def update_status(self, status, version=None):
        (self.status, self.status_colour) = {
            STATUS_NOT_INSTALLABLE: (
                QApplication.translate('Mod Status', 'Not applicable', None),
                None),
            STATUS_NOT_INSTALLED: (
                QApplication.translate('Mod Status', 'Not installed', None),
                None),
            STATUS_OLD_VERSION: (
                QApplication.translate('Mod Status', 'Old version installed',
                                       None), None),
            STATUS_INSTALLED: (
                QApplication.translate('Mod Status', 'Installed', None),
                QtGui.QColorConstants.DarkGreen),
            STATUS_NEWER_VERSION: (
                QApplication.translate('Mod Status', 'Installed', None),
                QtGui.QColorConstants.DarkGreen),
            STATUS_DESYNC: (QApplication.translate('Mod Status',
                                                   'alocalmod.rs5 not '
                                                   'synchronised',
                                                   None, ),
        ...
        if version is not None:
            self.status = QApplication.translate('Mod Status',
                                                 'Version {0} installed',
                                                 None, ).format(version)
...
class EnvMod(Mod):
    ...
    def refresh(self, miasmod_global_stat=None, miasmod_installed={}, **kwargs):
        ...
        return self.update_status_version(installed.get('version', None))
...
class Rs5Mod(Mod):
    ...
    def __init__(self, path):
        ...
        self.name = f'{self.mod_name} (main.rs5)'
    ...
    def data(self, index, role):
        ...
        if role == Qt.ForegroundRole:
            if not mod.installable:
                return QtGui.QBrush(QtGui.QColorConstants.Gray)
        ...
        if role == Qt.CheckStateRole:
            if index.column() == 0 and mod.installable:
                return (mod.install and Qt.CheckState.Checked or
...
class MiasPatch(QDialog):
    ...
    def process_install_path(self, path):
        ...
        def bad_install_path(file):
            ...
            dialog.setText(self.tr(
                f'{path} does not appear to be a Miasmata install: {file} not '
                f'found'))
            return dialog.exec_()
    ...
    def disable_install_button(self):
        self.ui.patch_game.setStyleSheet(
    ...
    def enumerate_bin_patch(self):
        ...
        for patch in patches.split():
            mod = import_module(f"patches.{patch}")
            if hasattr(mod, 'init'):
                mod.init(config)
    ...
    def resize_patch_list(self):
        ...
        def sizeHint():
            ...
            for col in range(cols):
                ...
                if col == cols - 1 and self.ui.patch_list.horizontalHeader(

                ).stretchLastSection():
                    col_width = max(col_width,
                                    self.ui.patch_list.horizontalHeader(
    ...
    def on_browse_clicked(self):
        dialog = QFileDialog(self,
                             caption=self.tr(
                                 "Select Miasmata Install Location..."))
    ...
    def progress_extra(self, prefix='', min=0, max=100, progress=None):
        ...
        def progress_update(msg=None, percent=None):
            if msg is not None:
                msg = f'{prefix}{msg}'
            if percent is not None:
        ...
        for file in files:
            ...
            if os.path.isfile(path):
                ...
                try:
                    ...
                    self.progress(
                        msg=self.tr(
                            '{0} occurred while removing {1}: {2}').format(
                            e.__class__.__name__, path, str(e)))
    ...
    def refresh_patch_list(self):
        ...
        self.patch_list.refresh(main_rs5=self.main_rs5,
                                miasmod_global_stat=global_stat,
                                miasmod_installed=installed)
        self.update_install_button()
        self.resize_patch_list()
    ...
    def install_bin_mods(self, progress):
        mods = [x for x in self.patch_list if
                x.install and isinstance(x, BinMod)]
        steps = len(mods)
        for (i, mod) in enumerate(mods):
            p = self.progress_extra(f'{mod.name}: ', min=i * 100 / steps,
                                    max=(i + 1) * 100 / steps,
                                    progress=progress)
    ...
    def install_rs5_mods(self, progress):
        mods = [x for x in self.patch_list if
                x.install and isinstance(x, Rs5Mod)]
        if len(mods) == 0:
            return
        ...
        i = 0
        for mod in mods:
            p = self.progress_extra('%s: ' % mod.name, min=i * 100 / steps,
                                    max=(i + mod.steps()) * 100 / steps,
                                    progress=progress)
            mod.install_mod(p, main_rs5=self.main_rs5)
            i += mod.steps()
        ...
        try:
            ...
            print(f'Prefixing mod order with: {prefix_order}')
            try:
                order = rs5mod.ModOrderDecoder(self.main_rs5)
            except Exception as e:
                order = []
            print(f'Old mod order: {order}')
    ...
    def get_installed_miasmods(self):
        ...
        mods = dict(
            [(os.path.splitext(os.path.basename(path))[0], path) for path in
             installed])
    ...
    def miasmod_order(mods):
        ...
        # environment.rs5, communitypatch.miasmod, sorted(*.miasmod),
        # alocalmod.miasmod
        order = sorted(mods)
    ...
    def process_miasmods(self, mods, order, copy_mods, progress, env=None):
        ...
        for (i, mod) in enumerate(order):
            progress(percent=i * 100 / steps,
                     msg=self.tr('Processing {0}...').format(mod))

            path = mods[mod]
            if isinstance(path, EnvMod):
                assert copy_mods
                try:
                    shutil.copyfile(path.path, os.path.join(self.install_path,
                                                            '%s.miasmod' % mod))
                except Exception as e:
                    progress(msg=self.tr(
                        '{0} occurred while copying {1}: {2}').format(
                        e.__class__.__name__, path.path,
                        str(e)))
                path = path.path
            ...
            try:
                ...
                progress(
                    msg=self.tr(
                        '{0} occurred while processing {1}: {2}').format(
                        e.__class__.__name__, path, str(e)))
            else:
    ...
    def check_installed_miasmods(self):
        ...
        (env, installed) = self.process_miasmods(mods, order, False,
                                                 self.progress, env)
    ...
    def install_env_mods(self, progress):
        bundled_mods = [x for x in self.patch_list if
                        x.install and isinstance(x, EnvMod)]

        (mods, mod_states) = self.get_installed_miasmods()
        if len(mods) and not os.path.isfile(
                os.path.join(self.install_path, 'alocalmod.rs5')):
            ...
            mod_states.update(
                dict(list(zip(list(mods.keys()), [False] * len(mods)))))
        ...
        if mod_states is not None:
            ...
            try:
                mod_states_path = miasmod.conf_path(self.install_path)
                json.dump(mod_states, open(mod_states_path, 'w'),
                          ensure_ascii=True)
            except IOError as e:
                progress(
                    msg=self.tr(
                        '{0} occurred while writing to {1}: {2}').format(
                        e.__class__.__name__, mod_states_path,
                        str(e)))
        ...
        print('\n'.join(map(str, iter(mods.values()))))
        print(f'In this order: {' '.join(order)}')

        (env, installed) = self.process_miasmods(mods, order, True, progress)
        environment.encode_to_archive(env, os.path.join(self.install_path,
                                                        'alocalmod.rs5'))
...
def start_gui_process(pipe=None):
    ...
    try:
        ...
        translator.load(f'miaspatch_i18n/{language}')
