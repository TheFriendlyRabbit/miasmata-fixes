...
def warn_unsupported_encoding(orig, filtered, illegal):
    ...
    dialog.setInformativeText('Some characters could not be encoded in'
                              ' Windows-1252 used by Miasmata and have been '
                              'removed.'
                              ' Please refer to this page for a table of '
                              'supported'
                              ' characters:'
                              ' https://en.wikipedia.org/wiki/Windows-1252')
    dialog.setDetailedText('Your text:\n{0}\ncontained the following'
                           ' unsupported characters:\n{1}\nIt was filtered'
                           ' to:\n{2}'.format(orig, ' '.join(illegal),
                                              filtered))
    ret = dialog.exec_()
...
class MiasmataDataModel(QtCore.QAbstractItemModel):
    ...
    def index(self, row, column, parent):
        ...
        child = list(parent_node.values())[row]
    ...
    def parent(self, index):
        ...
        if parent_node is self.root:
            return QtCore.QModelIndex()
        parent_row = list(parent_node.parent.keys()).index(parent_node.name)
    ...
    def removeRows(self, row, count, parent):
        ...
        while count:
            del parent_node[list(parent_node.keys())[row]]
            count -= 1
        self.endRemoveRows()
        return True
    ...
    def flags(self, index):
        ...
        if index.column() == 1:
            node = self.index_to_node(index)
            if isinstance(node,
                          (data.NullStr, data.DataInt, data.DataFloat)):
...
class MiasmataDataSortProxy(QSortFilterProxyModel):
    def index_to_node(self, index):
...
class MiasmataDataMixedListModel(MiasmataDataListModel):
    def data(self, index, role):
        ...
        if isinstance(item, data.NullStr):
            return '"%s"' % str(item)
        return str(item)
    ...
    def setData(self, index, value, role):
        ...
        if len(value) >= 2 and value[0] == value[-1] == '"':
            return self._setData(index, data.NullStr(value[1:-1]))
        try:
            new_item = data.DataInt(value)
        except ValueError:
            try:
                new_item = data.DataFloat(value)
            except ValueError:
                new_item = data.NullStr(value)
        return self._setData(index, new_item)
...
class MiasmataDataView(QWidget):
    ...
    def __init__(self, root, sort=True, save_path=None, diff_base=None,
                 miasmod_path=None, rs5_path=None, parent=None,
        ...
        if sort:
            ...
            # self.ui.search.textChanged.connect(
            # self.sort_proxy.setFilterFixedString)
        ...
        items = [type.desc for type in list(data.data_types.values())]
    ...
    def update_view(self, current, previous):
        ...
        self.ui.type.setCurrentIndex(
            list(data.data_types.keys()).index(node.id))
        ...
        if isinstance(node, data.DataList):
            if isinstance(node, data.DataMixedList):
        ...
        if isinstance(node, (data.NullStr, data.DataInt, data.DataFloat)):
            self.ui.value_line.setText(str(node))
            if isinstance(node, data.DataInt):
                validator = QtGui.QIntValidator(self)
            elif isinstance(node, data.DataFloat):
    ...
    def write_saves_dat(self):
        ...
        try:
            ...
            QMessageBox.warning(self, 'MiasMod',
                                '%s while encoding data\n\n%s\n\nRefusing to '
                                'write saves.dat!\n\nThis means there is a '
                                'bug in MiasMod, please report this to '
                                'DarkStarSword!' \
        ...
        try:
            ...
            try:
                ...
                QMessageBox.information(self, 'MiasMod',
                                        'Succesfully restored backup')
            except:
                QMessageBox.warning(self, 'MiasMod',
                                    '%s while restoring %s\n\n%s' \
                                    % (e.__class__.__name__, backup,
                                       str(e)))
            return
        return True
    ...
    def on_type_activated(self):
        new_type = list(data.data_types.values())[self.ui.type.currentIndex()]
    ...
    def insert_node(self, node, name_prefix):
        ...
        i = 0
        while True:
            name = data.NullStr('%s_%i' % (name_prefix, i))
    ...
    def on_actionInsert_Row_triggered(self):
        ...
        self.ui.value_list.edit(index, QAbstractItemView.AllEditTriggers,
                                None)
