...
def padding(pos, alignment):
    return b'\0' * padding_len(pos, alignment)


def parse_rs5file_header(f):
    # Based on parse_inod_header
    (magic, u1, filename_len, u2, filesize) = struct.unpack('<4s2sBBI',
                                                            f.read(12))
    assert u1 == b'\0\0'
    filename = f.read(filename_len).rstrip(b'\0').decode("utf-8")
    # print>>sys.stderr, 'Parsing header %s %s (u=%i)...' % (magic, filename,
    # u2)
...
def enc_header(magic, name, size, u2):
    name = name + b'\0'
    ret = struct.pack('<4s2sBBI', magic.encode("utf-8"), b'\0\0', len(name), u2,
                      size)
...
def enc_file(magic, name, data, u2):
    ...
    if isinstance(data, str):
        return header + data.encode("utf-8") + padding(len(data), 8)
    else:
...
class Rs5ChunkDecoder(Rs5Chunk):
    def __init__(self, fp):
        ...
        assert fp.read(pad_len) == b'\0' * pad_len
...
class Rs5FileDecoder(Rs5File):
    def __init__(self, data):
        ...
        (self.magic, self.filename, self.filesize,
         self.u2) = parse_rs5file_header(self.fp)
    ...
    def __init__(self, data):
        ...
        while True:
            try:
                ...
                self[chunk.name.decode("utf-8")] = chunk
            except EOFError:
                if self == {}:
                    raise
                break
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        ...
        print()
        print(f'usage: {sys.argv[0]} type filename > 00-HEADER')
        sys.exit(1)
