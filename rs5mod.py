...
try:
    ...
    class RS5Patcher(QtCore.QObject):
        pass
RS5Patcher = RS5Patcher()

undo_file = 'MIASMOD\\UNDO'
mod_manifests = 'MIASMOD\\MODS'
mod_order_file = 'MIASMOD\\ORDER'
mod_meta_file = 'MIASMOD\\MODINFO'
...
class UndoMetaEncoder(UndoMeta, rs5file.Rs5FileEncoder):
    def __init__(self, rs5):
        ...
        rs5file.Rs5FileEncoder.__init__(self, 'META', undo_file,
                                        self.json.dumps(self), 0)
...
def do_add_undo(rs5, overwrite=False, progress=progress):
    if undo_file in rs5 and not overwrite:
        ...
        if validate_undo(rs5):
            return 1
        progress(
            msg=RS5Patcher.tr('Undo metadata appears to be invalid, updating'))
    undo = UndoMetaEncoder(rs5)
    try:
        rs5.add_from_buf(undo.encode())
        rs5.save(progress=progress)
    except Exception as e:
        progress(msg=RS5Patcher.tr(
            f'ERROR: {e.__class__.__name__} occured while adding undo '
            f'metadata: {e}'))
        progress(msg=RS5Patcher.tr('REVERTING CHANGES...'))
        undo.revert_rs5(rs5)
        print('\n')
        progress(msg=RS5Patcher.tr('FILE RESTORED'))
        raise
...
class ModCentralDirectoryEncoder(rs5archive.Rs5CentralDirectoryEncoder,
                                 rs5file.Rs5FileEncoder):
    def __init__(self, name, ent_len):
        self.ent_len = ent_len
        self.flags = 0
        rs5archive.Rs5CentralDirectoryEncoder.__init__(self)
        self.filename = f'{mod_manifests}\\{name}.manifest'

    def encode(self):
        from io import BytesIO
        self.fp = BytesIO()
        self.write_directory()
        rs5file.Rs5FileEncoder.__init__(self, 'META', self.filename,
                                        self.fp.getvalue(), 0)
...
class ModOrderEncoder(ModOrder, rs5file.Rs5FileEncoder):
    def __init__(self, rs5, order):
        ...
        rs5file.Rs5FileEncoder.__init__(self, 'META', mod_order_file,
                                        json.dumps(self), 0)
...
class ModOrderDecoder(ModOrder, rs5file.Rs5FileDecoder):
    def __init__(self, rs5):
        ...
        list.__init__(self, json.loads(self.data))


def rs5_mods(rs5):
    mods = [x for x in rs5 if
            x.startswith(f'{mod_manifests}\\') and x.endswith('.manifest')]
    if mod_order_file in rs5:
        order = ModOrderDecoder(rs5)
        for mod in order:
            manifest = f'{mod_manifests}\\{mod}.manifest'
            if manifest in rs5:
                # print('Processing %s (ordered)' % mod)
                yield mods.pop(mods.index(manifest))
            else:
                print(
                    f'WARNING: {mod} listed in {mod_order_file} not found in '
                    f'archive!')
    yield from mods
...
def iter_all_file_versions(rs5, undo):
    ...
    if undo is not None:
        ...
        for mod_name in mods:
            ...
            masked.update(list(zip(mod, [mod_name] * len(mod))))
        ...
        for file in UndoMetaCentralDirectory(rs5).values():
            yield process(file)
        for mod in mods:
            yield process(rs5[mod], mod)
            for file in ModCentralDirectoryDecoder(rs5, rs5[mod]).values():
                yield process(file, mod)

    for (filename,) in set(
            [(x.filename, x.data_off) for x in list(rs5.values())]).difference(
        done):
...
def iter_mod_file_versions(rs5, undo_pos):
    ...
    remaining = [filename_off for filename_off in remaining if
                 filename_off[1] >= undo_pos]
    for (filename, value) in remaining:
        yield rs5[filename.decode("utf-8")]
...
def iter_all_used_sections(rs5):
    ...
    yield (rs5.d_off, rs5.d_off + rs5.d_orig_len, '<CENTRAL DIRECTORY>', None,
           None)  # Central Directory
    if undo is not None and (undo['directory_offset'] != rs5.d_off or undo[
        'directory_size'] != rs5.d_orig_len):
        yield (undo['directory_offset'],
               undo['directory_offset'] + undo['directory_size'],
               '<ORIGINAL CENTRAL DIRECTORY>', None, 'other central directory')
    for (file, mod, masked) in iter_all_file_versions(rs5, undo):
        yield (file.data_off, file.data_off + file.compressed_size,
               file.filename, mod, masked)
...
def iter_used_sections(rs5):
    ...
    if rs5.d_off >= undo_pos:
        yield (rs5.d_off, rs5.d_off + rs5.d_orig_len,
               '__directory__')  # Central Directory
...
def list_holes(archive):
    ...
    for (i, (start, fin, name, mod, masked)) in enumerate(regions):
        if i:
            ...
            if space:
                print(f'<-- HOLE: {space} bytes -->')
                wasted += space
            if space < 0:
                print(
                    f' !!! WARNING: {-space} BYTES OF OVERLAPPING DATA '
                    f'DETECTED !!!')
                bad = True
        mask_str = mod_str = ''
        if mod is not None:
        ...
        if masked is not None:
            mask_str = f' (MASKED BY {masked})'
        print('%.8x:%.8x %s%s%s' % (start, fin, mod_str, name, mask_str))
    print()
    print(f'Total space wasted by holes: {wasted} bytes')
    assert (not bad)
...
class Rs5ModArchiveUpdater(rs5archive.Rs5ArchiveUpdater):
    ...
    def find_holes(self):
        ...
        if len(self.holes):
            print('\n'.join(
                'Hole found: %i bytes at 0x%x' % x for x in self.holes))
        else:
    ...
    def seek_find_hole(self, size):
        ...
        for (i, (hole_size, hole)) in enumerate(self.holes):
            if hole_size >= size:
                print('Filling hole at 0x%x of size %i with %i bytes' % (hole,
                                                                         hole_size,
                                                                         size))
                if hole_size == size:
                    del self.holes[i]
                    ...
                    self.holes.sort()  # Insertion sort would be more
                # efficient here
...
def do_rm_mod(rs5, mod, progress=progress):
    ...
    try:
        ...
        progress(msg=RS5Patcher.tr('ERROR: {0} not found in archive!').format(
            manifest_name))
        raise ModNotFound()
    for (filename, mod_f) in ModCentralDirectoryDecoder(rs5, manifest).items():
        cur_f = rs5[filename]
        if cur_f.data_off == mod_f.data_off:
            ...
            progress(
                msg=RS5Patcher.tr('Skipping {0} - offsets do not '
                                  'match').format(
                    filename))
...
def get_mod_meta(rs5, mod_name=None):
    ...
    if mod_name is not None:
        file = f'{mod_manifests}\\{mod_name}.modinfo'
...
def get_mod_name(rs5, filename):
    try:
        ...
        if 'NAME' in meta:
            return meta['NAME'].data.decode("utf-8").strip()
    return os.path.splitext(os.path.basename(filename))[0]


def do_get_mod_version(meta):
    if 'VRSN' in meta:
        return meta['VRSN'].data.decode("utf-8").strip()
...
def do_add_mod(dest_rs5, source_rs5, source_archive, progress=progress):
    ...
    for (i, source_file) in enumerate(source_rs5.values()):
        percent = i * 100 / len(source_rs5)
        if file_blacklisted(source_file.filename):
            progress(percent=percent, msg=RS5Patcher.tr('Skipping {0}').format(
                source_file.filename))
            continue
        progress(percent=percent, msg=RS5Patcher.tr('Adding {0}...').format(
            source_file.filename))
        entry = rs5archive.Rs5CompressedFileRepacker(dest_rs5.fp, source_file,
                                                     seek_cb=dest_rs5.seek_find_hole)
