...
def compose_button(button_txt_file, source_blank_image, output_basename):
    ...
    if output_basename.lower().startswith('yes'):
        off = 5
    layer = add_text_layer_from_file(image, button_txt_file, font,
                                     colour=(255, 255, 255))
    pdb.gimp_text_layer_set_letter_spacing(layer, -1.0)
    place_text(layer, off + (image.width - 7) / 2, image.height / 2 - 1,
...
register(
    ...
    None,
    [
        (PF_FILE, "button_txt_file",
         "utf-8 encoded file with the text to place on the button.", None),
