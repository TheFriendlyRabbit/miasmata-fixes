...
def compose_index_image(source_txt_file, source_blank_image, output_basename):
    ...
    text = add_text_layer_from_file(image, source_txt_file,
                                    font(output_basename))
    pdb.gimp_text_layer_set_justification(text, TEXT_JUSTIFY_CENTER)
    word_wrap_balanced(text, w)

    pdb.gimp_layer_translate(text, x + (w - text.width) / 2,
    ...
    layer_l = text.copy();
    image.add_layer(layer_l, 0);
    layer_l.translate(-1, 0)
    layer_r = text.copy();
    image.add_layer(layer_r, 0);
    layer_r.translate(1, 0)
    layer_v = text.copy();
    image.add_layer(layer_v, 0)
    ...
    pdb.plug_in_convmatrix(image, layer_v, 25, matrix, False, 1, 0, 5, channels,
                           2)
...
def compose_index_title_image(source_txt_name, source_blank_image,
                              output_basename):
    image = pdb.gimp_file_load(source_blank_image, source_blank_image)
