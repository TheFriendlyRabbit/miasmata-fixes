...
def compose_tags(image, item_name, output_basename):
    ...
    for y in range(4):
        for x in range(2):
            ...
            # place_text(layer, tag_x[x], y*tag_h + tag_h/2, w=tag_w,
...
def compose_lab_sampletrays(image, item_name, output_basename):
    labels = list(map(str.strip, read_text('%s.txt' % item_name).split('\n\n')))
    ...
    for y in range(2):
        for x in range(2):
            layer = add_text(image, labels.pop(0), sample_tray_font,
                             colour=(255, 255, 255))
            pdb.gimp_text_layer_set_justification(layer, TEXT_JUSTIFY_CENTER)
            word_wrap(layer, None, 450)
            place_text(layer, label_x[x], label_y[y], xalign=CENTER,
                       yalign=CENTER)
...
register(
    ...
    [
        (PF_FILE, "item_name",
         "Which item to compose. Each item has it's own requirements for "
