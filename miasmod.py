...
class ModList():
    class mod():
        ...
        def add(self, path, basename, type, note=None, include=True,
                version=None):
            if type == 'rs5':
                basename = f'{basename}/environment'
            setattr(self, f'{type}_path', path)
            setattr(self, f'{type}_name', basename)
    ...
    def __getitem__(self, idx):
        return (list(self.mods.values()) + list(self.mods_last.values()))[idx]
    ...
    def data(self, index, role):
        ...
        if role == Qt.ToolTipRole:
            if index.column() == 3:
                note = mod.note[1]
                if note is None and mod.name == self.mod_list.active:
                    return ('Based on the filename, Miasmata will use the '
                            'environment found in this RS5 file')
                return note
        ...
        if role == Qt.CheckStateRole:
            if index.column() == 0 and \
                ...
                return (mod.include and Qt.CheckState.Checked or
...
class Rs5ModListModel(QtCore.QAbstractTableModel):
    ...
    def refresh(self):
        ...
        self.beginResetModel()  # aura: was self.reset()
...
class MiasMod(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.ui.tabWidget.tabBar().tabButton(0, QTabBar.RightSide).resize(
            0, 0)
        self.open_tabs = {}
    ...
    def on_rs5_mod_list_customContextMenuRequested(self, pos):
        if not len(self.ui.rs5_mod_list.selectedIndexes()):
            return
        if self.ui.rs5_mod_list.columnAt(
                pos.x()) == -1 or self.ui.rs5_mod_list.rowAt(pos.y()) == -1:
            return
    ...
    def on_open_saves_dat_clicked(self):
        ...
        try:
            ...
            path = QFileDialog.getOpenFileName(self,
                                               "Select Miasmata Save "
                                               "Location...",
                                               None,
                                               'Miasmata save files (*.dat)')[
                0]
            if not path:
                return
        ...
        saves.name = data.NullStr('saves.dat')
    ...
    def warn_bad_miasmata_path(self, path):
        ...
        dialog.setInformativeText(f"Please check that '{path}' is the"
                                  " correct location of Miasmata")
        return dialog.exec()
    ...
    def refresh_mod_list(self):
        ...
        for path in sorted(glob(os.path.join(self.install_path, '*.rs5')),
                           reverse=True):
            basename = os.path.basename(path)
        ...
        mod_list.extend(
            sorted(glob(os.path.join(self.install_path, '*.miasmod'))),
            mod_states)
    ...
    def closeEvent(self, event):
        for i in range(1, self.ui.tabWidget.count()):
            if self.ask_save_tab(i) == QMessageBox.StandardButton.Cancel:
                event.ignore()
                return
        event.accept()
    ...
    def on_tabWidget_tabCloseRequested(self, index):
        if index == 0:
            return
        if self.ask_save_tab(index) == QMessageBox.StandardButton.Cancel:
            return
    ...
    def ask_edit(self, mod, msg1, msg2, edit_mod, edit_rs5, detailed=None):
        ...
        edit_mod = dialog.addButton(edit_mod, dialog.ButtonRole.AcceptRole)
        if edit_rs5 is not None:
            edit_rs5 = dialog.addButton(edit_rs5,
                                        dialog.ButtonRole.DestructiveRole)
    ...
    def ask_generate_mod_diff(self, row, mod, open=True):
        ...
        # msg3 = data.pretty_fmt_diff(data.diff_data(env1, env2), ' + '.join(
        # sources), mod.rs5_name)
        msg3 = None
    ...
    def add_tab(self, view, name, mod_name):
        ...
        self.ui.tabWidget.addTab(view, str(name))
    ...
    def open_mod_rs5_only(self, row, mod):
        ...
        view = miasmod_data.MiasmataDataView(env, name=mod.name,
                                             rs5_path=mod.rs5_path,
                                             version=mod.version)
    ...
    def open_alocalmod(self):
        ...
        view = miasmod_data.MiasmataDataView(env2, name=mod.name,
                                             diff_base=env1,
                                             miasmod_path=mod.miasmod_path,
                                             rs5_path=mod.rs5_path)
        self.add_tab(view, mod.name, mod.name)
    ...
    def on_new_mod_clicked(self):
        (path, filter) = QFileDialog.getSaveFileName(self,
                                                     caption="New Miasmata "
                                                             "mod...",
                                                     dir=self.install_path,
                                                     filter="MiasMod files ("
                                                            "*.miasmod)")
        if not path:
            return
        basename = os.path.basename(path)
        ...
        view = miasmod_data.MiasmataDataView(env2, name=name, diff_base=env1,
                                             miasmod_path=path)
