...
def parse_exposure_map(exposure):
    ...
    def recurse_fill(x, y, size, depth):
        ...
        if val & 0x1:
            outline_mask_draw.rectangle((x, y, x + size - 1, y + size - 1),
                                        fill=1)
        if val & 0x2:
            filledin_mask_draw.rectangle((x, y, x + size - 1, y + size - 1),
                                         fill=1)
        if val & 0x4:
            overlayinfo_mask_draw.rectangle((x, y, x + size - 1, y + size - 1),
                                            fill=1)
...
def compose_map(outline, outline_mask, filledin, filledin_mask, overlayinfo,
                overlayinfo_mask):
...
def gen_map(exposure, filledin, overlayinfo):
    (outline_mask, filledin_mask, overlayinfo_mask, extra) = parse_exposure_map(
        exposure)

    image = compose_map(None, outline_mask, filledin, filledin_mask,
                        overlayinfo, overlayinfo_mask)
...
def overlay_smap(image, shoreline, outline_mask, filledin_mask):
    ...
    for (total, (x, y)) in enumerate(smap.smap_iter(shoreline, image.size[0]),
                                     1):
        if filledin_pix[x, y]:
            r = g = b = 80
            revealed += 1
    ...
    print('%i / %i (%i%%) of the coast mapped' % (revealed, total,
                                                  revealed * 100 / total))
...
def main():
    ...
    overlayinfo = rs5file.Rs5ChunkedFileDecoder(
        archive['TEX\\Map_OverlayInfo'].decompress())
    print('Decoding Map_OverlayInfo...')
    overlayinfo = imag.open_rs5file_imag(overlayinfo, (1024, 1024), 'RGB')

    print(
        'Extracting player_map_achievements...')  # XXX: Also in environment.rs5
    shoreline = rs5file.Rs5ChunkedFileDecoder(
        archive['player_map_achievements'].decompress())

    print('Generating map...')
    (image, outline_mask, filledin_mask, overlayinfo_mask, extra) = gen_map(
        exposure_map, filledin, overlayinfo)
    ...
    new_exposure_map = make_exposure_map(outline_mask, filledin_mask,
                                         overlayinfo_mask, extra)
