...
spoiler_plant_colours = collections.OrderedDict((
    ...
    ('plant25', '8000b0'),
    # Giant Bloom (TEST IF THIS IS ACCESSIBLE ON AGENT Y ISLAND)
...
for k, v in list(spoiler_plant_colours.items()):
...
min_z_blacklist_whole_node = -1.0  # Detects orbital launch platform,
# now just explicitly blacklisting that
blacklist_inods = (
    64784, 64783, 16195,  # Corrupt orbital launch site East of Rigel
    36614, 36616,
    # Developer area in river West of spawn with hidden(?) notes, coords
    # 2720x3456 - 2752x3520. Unclear why these notes don't show in game,
    # but we don't want to include them in the randomizer.
    19911)  # Milo Island easter egg East of Desert. Coords 4852x6452 -
# 4940x6539
...
def get_plants_notes_list(env_rs5, skip_non_removable=True):
    ...
    for modelset, game_object in env_rs5['player']['pick_objects'].items():
        if not modelset.startswith('modelsets\\'):
            continue
        if not game_object.startswith('plant') and not game_object.startswith(
                'note'):
            continue
        ...
        game_objects_case_sensitive = {x.lower(): x for x in
                                       env_rs5['game_objects']}
        ...
        if skip_non_removable and removal_mode == 0:
            print('Skipping %s %s because removal mode is 0' % (game_object,
                                                                model_name))
            continue
...
def spoil(plants, spoiler_filename='spoiler.jpg'):
    ...
    if plants is None:
        plants = [x for x in list(models.values()) if x.startswith('plant')]
    elif not isinstance(plants, (list, tuple)):
        plants = (plants,)

    m = {k: v for (k, v) in list(models.items()) if v in plants}
    ...
    search_inst_ids = {inst_node_names.index(k): v for k, v in m.items() if
                       k in inst_node_names}
...
def generate_and_install_randomizer(seed=None):
    ...
    search_inst_ids = {}
    for model, game_object in models.items():
        if model not in inst_node_names:
            print(
                'NOTE: %s referenced in environment.rs5 not found in '
                'inst_header (no instances of this game_object in the map)' %
                model)
            continue
        ...
        if bucket.shuffle_mode == 1:
            ...
            for game_object in bucket.bucket:
                ...
                if item_instances:
                    ...
                    print(
                        'Skipping %s with 0 instances in the map' % game_object)
            ...
            for item_id in all_items_in_bucket:
                item_id_map[item_id] = bucket.random.choice(
                    bucket.bucket[note_bucket.pop()])
            assert (len(note_bucket) == 0)
        elif bucket.shuffle_mode == 2:
            keys, vals = list(
                map(list, list(zip(*list(bucket.bucket.items())))))
            bucket.random.shuffle(keys)
            bucket.bucket.update(dict(list(zip(keys, vals))))
            ...
            for game_object in bucket.bucket:
                ...
                print('%i instances of %s grouped into %i clusters' % (
                    initial_len, game_object, final_len))
            ...
            for game_object in bucket.bucket:
                ...
                for cluster in all_clusters_in_bucket[:num_clusters]:
                    for plant_id in cluster.contents:
                        item_id_map[plant_id] = bucket.random.choice(
